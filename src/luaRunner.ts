...
<<<<<<< HEAD
  ...
  textWrapped?: boolean;
  font?: string;
  textXAlignment?: 'Left' | 'Center' | 'Right' | string;
  textYAlignment?: 'Top' | 'Center' | 'Bottom' | string;
  // Image Properties (ImageLabel, ImageButton)
  image?: string;
  imageColor?: string;
  imageTransparency?: number;
  scaleType?: 'Stretch' | 'Slice' | 'Tile' | 'Fit' | 'Crop' | string;
  // Button Properties (TextButton, ImageButton)
  ...
  displayOrder?: number;
  // SurfaceGui & BillboardGui Properties
  canvasSize?: [number, number];
  studsOffset?: [number, number, number];
  alwaysOnTop?: boolean;
  maxDistance?: number;
  adorneeId?: string;
...
<<<<<<< HEAD
    if (value.__type === 'UDim2' && ctx.bridge?.makeUDim2) {
      return ctx.bridge.makeUDim2(
        ...
        Number(value.X?.Offset ?? value.xOffset ?? 0),
        Number(value.Y?.Scale ?? value.yScale ?? 0),
        Number(value.Y?.Offset ?? value.yOffset ?? 0)
      );
    }
    if (value.__type === 'UDim' && ctx.bridge?.makeUDim) {
      return ctx.bridge.makeUDim(Number(value.Scale ?? value.scale ?? 0), Number(value.Offset ?? value.offset ?? 0));
    }
...
export function makeInstanceUDim2(
  ...
  const p = pos || { xScale: 0, xOffset: 0, yScale: 0, yOffset: 0 };
  if (bridge?.makeUDim2) {
    return bridge.makeUDim2(p.xScale, p.xOffset, p.yScale, p.yOffset);
  }
  const xDim = { __type: 'UDim', Scale: p.xScale, Offset: p.xOffset, scale: p.xScale, offset: p.xOffset };
  const yDim = { __type: 'UDim', Scale: p.yScale, Offset: p.yOffset, scale: p.yScale, offset: p.yOffset };
  return {
    __type: 'UDim2',
    X: xDim,
    Y: yDim,
    Width: xDim,
    Height: yDim,
    toString: () => `{${p.xScale}, ${p.xOffset}}, {${p.yScale}, ${p.yOffset}}`,
  };
}

export function makeInstanceVector2(bridge: LuaBridge | undefined, x = 0, y = 0): any {
  if (bridge?.makeVector2) {
    return bridge.makeVector2(x, y);
  }
  return {
    __type: 'Vector2',
    X: x,
    ...
    Unit: Math.sqrt(x * x + y * y) === 0 ? { __type: 'Vector2', X: 0, Y: 0, x: 0, y: 0 } : {
      __type: 'Vector2',
...
<<<<<<< HEAD
  ...
  const makeInstanceSignal = (signalName: string) => ({
    __type: 'RBXScriptSignal',
    Connect: (fn: any) => {
      if (typeof fn !== 'function') return null;
      const listener = (...args: any[]) => {
        if (signalName === 'Touched' || signalName === 'TouchEnded') {
          const otherNodeId = args[0];
...
<<<<<<< HEAD
    ...
    get MouseButton1Down() {
      return makeInstanceSignal('MouseButton1Down');
    },
    get MouseButton1Up() {
      return makeInstanceSignal('MouseButton1Up');
    },
    get Activated() {
      return makeInstanceSignal('Activated');
    },
    get MouseEnter() {
      return makeInstanceSignal('MouseEnter');
    },
    get MouseLeave() {
      return makeInstanceSignal('MouseLeave');
    },
    get FocusLost() {
...
<<<<<<< HEAD
      ...
      if (node.type === 'textbutton') return 'TextButton';
      if (node.type === 'imagelabel') return 'ImageLabel';
      if (node.type === 'imagebutton') return 'ImageButton';
      if (node.type === 'textbox') return 'TextBox';
...
<<<<<<< HEAD
      if (isGuiNode()) {
        ...
        if (v && v.__type === 'UDim2') {
          xs = Number(v.X?.Scale ?? 0);
          xo = Number(v.X?.Offset ?? 0);
          ys = Number(v.Y?.Scale ?? 0);
          yo = Number(v.Y?.Offset ?? 0);
        } else if (v && typeof v === 'object') {
          xs = Number(v.xScale ?? v.X?.Scale ?? 0);
          xo = Number(v.xOffset ?? v.X?.Offset ?? 0);
          ys = Number(v.yScale ?? v.Y?.Scale ?? 0);
          yo = Number(v.yOffset ?? v.Y?.Offset ?? 0);
        }
        updateTree((tree) => {
          const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
            nodes.map((n) => (n.id === nodeId ? { ...n, guiPosition: { xScale: xs, xOffset: xo, yScale: ys, yOffset: yo } } : { ...n, children: update(n.children) }));
          return update(tree);
        });
        return;
      }
...
<<<<<<< HEAD
    ...
    set AnchorPoint(v: any) {
      const x = Number(v?.X ?? v?.x ?? (Array.isArray(v) ? v[0] : 0)) || 0;
      const y = Number(v?.Y ?? v?.y ?? (Array.isArray(v) ? v[1] : 0)) || 0;
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, anchorPoint: [x, y] } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get BackgroundColor3() {
      return hexToColor3Value(getNode()?.backgroundColor || '#ffffff', bridge);
    },
    set BackgroundColor3(v: any) {
      const hex = colorValueToHex(v, '#ffffff');
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, backgroundColor: hex } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get BackgroundTransparency() {
      return getNode()?.backgroundTransparency ?? 0;
    },
    set BackgroundTransparency(v: number) {
      const val = Math.max(0, Math.min(1, Number(v) || 0));
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, backgroundTransparency: val } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get BorderColor3() {
      return hexToColor3Value(getNode()?.borderColor || '#000000', bridge);
    },
    set BorderColor3(v: any) {
      const hex = colorValueToHex(v, '#000000');
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, borderColor: hex } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get BorderSizePixel() {
      return getNode()?.borderSizePixel ?? 1;
    },
    set BorderSizePixel(v: number) {
      const val = Math.max(0, Math.round(Number(v) || 0));
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, borderSizePixel: val } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },
    ...
    set Visible(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, visible: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get Active() {
      return !!getNode()?.active;
    },
    set Active(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, active: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get ClipsDescendants() {
      return !!getNode()?.clipsDescendants;
    },
    set ClipsDescendants(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, clipsDescendants: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get LayoutOrder() {
      return getNode()?.layoutOrder ?? 0;
    },
    set LayoutOrder(v: number) {
      const val = Number(v) || 0;
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, layoutOrder: val } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },
    ...
    set Text(v: any) {
      ...
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, text: str } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get TextColor3() {
      return hexToColor3Value(getNode()?.textColor || '#000000', bridge);
    },
    set TextColor3(v: any) {
      const hex = colorValueToHex(v, '#000000');
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, textColor: hex } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get TextTransparency() {
      return getNode()?.textTransparency ?? 0;
    },
    set TextTransparency(v: number) {
      const val = Math.max(0, Math.min(1, Number(v) || 0));
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, textTransparency: val } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get TextSize() {
      return getNode()?.textSize ?? 14;
    },
    set TextSize(v: number) {
      const val = Math.max(1, Math.round(Number(v) || 14));
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, textSize: val } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get TextScaled() {
      return !!getNode()?.textScaled;
    },
    set TextScaled(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, textScaled: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get TextWrapped() {
      return !!getNode()?.textWrapped;
    },
    set TextWrapped(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, textWrapped: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get Font() {
      return getNode()?.font ?? 'SourceSans';
    },
    set Font(v: any) {
      const str = typeof v === 'object' && v?.Name ? String(v.Name) : String(v || 'SourceSans');
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, font: str } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get TextXAlignment() {
      return getNode()?.textXAlignment ?? 'Center';
    },
    set TextXAlignment(v: any) {
      const str = typeof v === 'object' && v?.Name ? String(v.Name) : String(v || 'Center');
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, textXAlignment: str } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get TextYAlignment() {
      return getNode()?.textYAlignment ?? 'Center';
    },
    set TextYAlignment(v: any) {
      const str = typeof v === 'object' && v?.Name ? String(v.Name) : String(v || 'Center');
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, textYAlignment: str } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },
    ...
    set Image(v: any) {
      ...
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, image: str } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get ImageColor3() {
      return hexToColor3Value(getNode()?.imageColor || '#ffffff', bridge);
    },
    set ImageColor3(v: any) {
      const hex = colorValueToHex(v, '#ffffff');
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, imageColor: hex } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get ImageTransparency() {
      return getNode()?.imageTransparency ?? 0;
    },
    set ImageTransparency(v: number) {
      const val = Math.max(0, Math.min(1, Number(v) || 0));
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, imageTransparency: val } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get ScaleType() {
      return getNode()?.scaleType ?? 'Stretch';
    },
    set ScaleType(v: any) {
      const str = typeof v === 'object' && v?.Name ? String(v.Name) : String(v || 'Stretch');
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, scaleType: str } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get AutoButtonColor() {
      return getNode()?.autoButtonColor !== false;
    },
    set AutoButtonColor(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, autoButtonColor: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },
    ...
    set Selected(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, selected: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get PlaceholderText() {
      return getNode()?.placeholderText ?? '';
    },
    set PlaceholderText(v: any) {
      const str = String(v ?? '');
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, placeholderText: str } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get ClearTextOnFocus() {
      return getNode()?.clearTextOnFocus !== false;
    },
    set ClearTextOnFocus(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, clearTextOnFocus: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get CanvasSize() {
      const cs = getNode()?.canvasSize || [800, 600];
      return makeInstanceVector2(bridge, cs[0], cs[1]);
    },
    set CanvasSize(v: any) {
      const x = Number(v?.X ?? v?.x ?? (Array.isArray(v) ? v[0] : 800)) || 800;
    ...
    get StudsOffset() {
      const so = getNode()?.studsOffset || [0, 2, 0];
      return makeInstanceVector3(bridge, so[0], so[1], so[2]);
    },
    set StudsOffset(v: any) {
      const x = Number(v?.X ?? v?.x ?? (Array.isArray(v) ? v[0] : 0)) || 0;
      const y = Number(v?.Y ?? v?.y ?? (Array.isArray(v) ? v[1] : 2)) || 2;
      const z = Number(v?.Z ?? v?.z ?? (Array.isArray(v) ? v[2] : 0)) || 0;
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, studsOffset: [x, y, z] } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get AlwaysOnTop() {
      return !!getNode()?.alwaysOnTop;
    },
    set AlwaysOnTop(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, alwaysOnTop: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get MaxDistance() {
      return getNode()?.maxDistance ?? 1000;
    },
    set MaxDistance(v: number) {
      const val = Math.max(0, Number(v) || 0);
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, maxDistance: val } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },
    ...
    set IgnoreGuiInset(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, ignoreGuiInset: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get ResetOnSpawn() {
      return getNode()?.resetOnSpawn !== false;
    },
    set ResetOnSpawn(v: boolean) {
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, resetOnSpawn: !!v } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },

    get DisplayOrder() {
      return getNode()?.displayOrder ?? 0;
    },
    set DisplayOrder(v: number) {
      const val = Number(v) || 0;
      updateTree((tree) => {
        const update = (nodes: TreeNodeData[]): TreeNodeData[] =>
          nodes.map((n) => (n.id === nodeId ? { ...n, displayOrder: val } : { ...n, children: update(n.children) }));
        return update(tree);
      });
    },
...
<<<<<<< HEAD
      ...
      if (lower === 'decal' && (nodeType === 'decal' || nodeType === 'texture')) return true;
      if (lower === 'texture' && nodeType === 'texture') return true;
      if (lower === 'guiinstance' || lower === 'guibase2d') {
        return ['screengui', 'frame', 'textlabel', 'textbutton', 'imagelabel', 'imagebutton', 'textbox', 'scrollingframe', 'surfacegui', 'billboardgui'].includes(nodeType);
      }
      if (lower === 'guiobject') {
        return ['frame', 'textlabel', 'textbutton', 'imagelabel', 'imagebutton', 'textbox', 'scrollingframe'].includes(nodeType);
      }
      if (lower === 'guibutton') {
        return ['textbutton', 'imagebutton'].includes(nodeType);
      }
      if (lower === 'layercollector') {
        return ['screengui', 'surfacegui', 'billboardgui'].includes(nodeType);
      }
      if (lower === 'frame' && (nodeType === 'frame' || nodeType === 'scrollingframe')) return true;
      return (
        nodeType === lower ||
...
<<<<<<< HEAD
  ...
  const getStarterGuiProxy = () => {
    ...
    return starterGui ? createInstanceProxy(starterGui.id, getTree, updateTree, onLog, bridge) : null;
  };

  const localPlayerProxy: any = {
    __type: 'Player',
    Name: 'Player1',
    UserId: 1,
    DisplayName: 'Player1',
    Character: null,
    get PlayerGui() {
      return getStarterGuiProxy();
    },
    WaitForChild: (childName: string) => {
      if (childName.toLowerCase() === 'playergui') {
        return getStarterGuiProxy();
      }
      return null;
    },
  ...
  services.players = {
    ...
    ClassName: 'Players',
    get LocalPlayer() {
      return bridge?.side === 'client' ? localPlayerProxy : null;
    },
    GetPlayers: () => [localPlayerProxy],
    FindFirstChild: (name: string) => (name === 'Player1' ? localPlayerProxy : null),
    WaitForChild: (name: string) => (name === 'Player1' ? localPlayerProxy : null),
    PlayerAdded: makeHubSignal('PlayerAdded'),
...
<<<<<<< HEAD
        ...
        function Vector2.__mul(a, b)
          ...
          elseif type(a) == "table" and type(b) == "table" and a.__type == "Vector2" and b.__type == "Vector2" then
            return Vector2.new(a.X * b.X, a.Y * b.Y)
          else
            error("Attempt to multiply Vector2 with incompatible type")
          end
        end
        ...
        function Vector2.__div(a, b)
          if type(b) == "number" then
            return Vector2.new(a.X / b, a.Y / b)
          elseif type(b) == "table" and b.__type == "Vector2" then
            return Vector2.new(a.X / b.X, a.Y / b.Y)
          else
            error("Attempt to divide Vector2 with incompatible type")
          end
        end
        ...
        local Vector2_meta_index = function(t, k)
          if k == "Magnitude" or k == "magnitude" then
            return math.sqrt(t.X * t.X + t.Y * t.Y)
          elseif k == "Unit" or k == "unit" then
            local m = math.sqrt(t.X * t.X + t.Y * t.Y)
            if m == 0 then return Vector2.zero end
        ...
        Vector2.__index = Vector2_meta_index
        _G.Vector2 = Vector2
        ...
        function UDim.__tostring(u)
          return string.format("%.3f, %d", u.Scale, u.Offset)
        end
        UDim.__index = UDim
        _G.UDim = UDim
        ...
        function UDim2.new(xs, xo, ys, yo)
          local x, y
          if type(xs) == "table" and xs.__type == "UDim" and type(xo) == "table" and xo.__type == "UDim" then
            x = xs
            y = xo
          else
            x = UDim.new(xs, xo)
        ...
        function UDim2:Lerp(target, alpha)
          local x = self.X.Scale + (target.X.Scale - self.X.Scale) * alpha
          local xo = self.X.Offset + (target.X.Offset - self.X.Offset) * alpha
          local y = self.Y.Scale + (target.Y.Scale - self.Y.Scale) * alpha
          local yo = self.Y.Offset + (target.Y.Offset - self.Y.Offset) * alpha
          return UDim2.new(x, xo, y, yo)
        end

        local UDim2_meta_index = function(t, k)
          if k == "Width" then return t.X
          elseif k == "Height" then return t.Y
          elseif k == "xScale" then return t.X.Scale
          elseif k == "xOffset" then return t.X.Offset
          elseif k == "yScale" then return t.Y.Scale
          elseif k == "yOffset" then return t.Y.Offset
          else return UDim2[k] end
        end
...
<<<<<<< HEAD
        ...
        function __MAKE_UDIM(scale, offset)
...
<<<<<<< HEAD
            elseif valueKind == "UDim2" then
              ...
              local ys = (value.Y and (value.Y.Scale or value.Y.scale)) or value.yScale or 0
              local yo = (value.Y and (value.Y.Offset or value.Y.offset)) or value.yOffset or 0
              return UDim2.new(xs, xo, ys, yo)
            elseif valueKind == "UDim" then
...
<<<<<<< HEAD
              ...
              if (key == "Position" or key == "Size") and value ~= nil and type(value) == "table" and value.__type == "UDim2" then
                local xs = (value.X and (value.X.Scale or value.X.scale)) or value.xScale or 0
                local xo = (value.X and (value.X.Offset or value.X.offset)) or value.xOffset or 0
                local ys = (value.Y and (value.Y.Scale or value.Y.scale)) or value.yScale or 0
                local yo = (value.Y and (value.Y.Offset or value.Y.offset)) or value.yOffset or 0
...
<<<<<<< HEAD
        ...
        const isGui = ['screengui', 'frame', 'textlabel', 'textbutton', 'imagelabel', 'imagebutton', 'textbox', 'scrollingframe', 'surfacegui', 'billboardgui'].includes(nodeType);

        let parentId = isGui && nodeType === 'screengui' ? 'startergui' : 'workspace';
...
<<<<<<< HEAD
          ...
          // GUI object properties
          guiPosition: isGui ? { xScale: 0, xOffset: 0, yScale: 0, yOffset: 0 } : undefined,
          guiSize: isGui
            ? (['textlabel', 'textbutton', 'textbox', 'billboardgui'].includes(nodeType)
          ...
          textColor: nodeType === 'textbutton' ? '#ffffff' : isGui ? '#000000' : undefined,
          textSize: isGui ? 14 : undefined,
          font: isGui ? 'SourceSans' : undefined,
          textXAlignment: isGui ? 'Center' : undefined,
          textYAlignment: isGui ? 'Center' : undefined,
          autoButtonColor: ['textbutton', 'imagebutton'].includes(nodeType),
          placeholderText: nodeType === 'textbox' ? 'Type here...' : undefined,
          clearTextOnFocus: nodeType === 'textbox' ? true : undefined,
          image: ['imagelabel', 'imagebutton'].includes(nodeType) ? '' : undefined,
