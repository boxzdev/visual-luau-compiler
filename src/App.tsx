...
function hexToRgba(hex?: string, transparency = 0): string {
  if (!hex) return `rgba(255, 255, 255, ${Math.max(0, 1 - transparency)})`;
...
function getFontFamily(font?: string): string {
  ...
  if (f.includes('mono') || f === 'code') return 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace';
  if (f.includes('gotham')) return 'Montserrat, "Segoe UI", sans-serif';
  if (f.includes('arial')) return 'Arial, Helvetica, sans-serif';
  return '"Source Sans Pro", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
}

function useRobloxImage(input?: string): string | null {
  ...
  useEffect(() => {
    ...
    resolveRobloxAssetUrl(raw).then((candidates) => {
...
type GuiUDim2 = { xScale: number; xOffset: number; yScale: number; yOffset: number };
type GuiResizeHandle = 'move' | 'n' | 's' | 'e' | 'w' | 'ne' | 'nw' | 'se' | 'sw';

function GuiElementNode({
  node,
  isInteractive,
  onUpdateText,
  editable = false,
  selectedGuiId = null,
  onSelectGui,
  onGuiTransformChange,
}: {
  node: TreeNodeData;
  isInteractive: boolean;
  onUpdateText?: (nodeId: string, text: string) => void;
  // When true, the "Move/Resize GUI" tool is active for this overlay: GUI
  // elements become clickable so they can be dragged to move, or dragged
  // by an edge/corner handle to resize.
  editable?: boolean;
  selectedGuiId?: string | null;
  onSelectGui?: (nodeId: string) => void;
  onGuiTransformChange?: (nodeId: string, guiPosition: GuiUDim2, guiSize: GuiUDim2) => void;
}) {
  const [isHovered, setIsHovered] = useState(false);
  const [isPressed, setIsPressed] = useState(false);
  // Live position/size while a move/resize drag is in progress, so the
  // element tracks the cursor smoothly before the change is committed.
  const [liveTransform, setLiveTransform] = useState<{ pos: GuiUDim2; size: GuiUDim2 } | null>(null);
  const typeLower = (node.type || '').toLowerCase();
  const visible = node.visible !== false;

  const imageUrl = useRobloxImage(node.image);

  if (!visible) return null;

  const pos = liveTransform?.pos || node.guiPosition || { xScale: 0, xOffset: 0, yScale: 0, yOffset: 0 };
  const sz = liveTransform?.size || node.guiSize || { xScale: 0, xOffset: 100, yScale: 0, yOffset: 100 };
  const anchor = node.anchorPoint || [0, 0];
  const isSelectedForTransform = editable && selectedGuiId === node.id;

  // Starts a move (whole-body) or resize (edge/corner handle) drag. Mouse
  // deltas are added directly to the pixel offset of position/size, since
  // the overlay renders at a 1:1 pixel scale with no zoom transform.
  const startGuiDrag = (e: React.PointerEvent, handle: GuiResizeHandle) => {
    e.stopPropagation();
    e.preventDefault();
    onSelectGui?.(node.id);

    // Route all subsequent events for this gesture through this element,
    // even if the finger/cursor moves off it - required for touch, since
    // touch has no free-floating window-level move events like mouse does.
    (e.target as Element).setPointerCapture?.(e.pointerId);

    const startX = e.clientX;
    const startY = e.clientY;
    const startPos: GuiUDim2 = { ...pos };
    const startSize: GuiUDim2 = { ...sz };
    const MIN_SIZE = 12;

    const onPointerMove = (ev: PointerEvent) => {
      const dx = ev.clientX - startX;
      const dy = ev.clientY - startY;
      const newPos: GuiUDim2 = { ...startPos };
      const newSize: GuiUDim2 = { ...startSize };

      if (handle === 'move') {
        newPos.xOffset = startPos.xOffset + dx;
        newPos.yOffset = startPos.yOffset + dy;
      } else {
        if (handle.includes('e')) {
          newSize.xOffset = Math.max(MIN_SIZE, startSize.xOffset + dx);
        }
        if (handle.includes('s')) {
          newSize.yOffset = Math.max(MIN_SIZE, startSize.yOffset + dy);
        }
        if (handle.includes('w')) {
          const clampedDx = Math.min(dx, startSize.xOffset - MIN_SIZE);
          newSize.xOffset = startSize.xOffset - clampedDx;
          newPos.xOffset = startPos.xOffset + clampedDx;
        }
        if (handle.includes('n')) {
          const clampedDy = Math.min(dy, startSize.yOffset - MIN_SIZE);
          newSize.yOffset = startSize.yOffset - clampedDy;
          newPos.yOffset = startPos.yOffset + clampedDy;
        }
      }

      setLiveTransform({ pos: newPos, size: newSize });
    };
    ...
    const onPointerUp = () => {
      ...
      setLiveTransform((current) => {
        if (current) {
          onGuiTransformChange?.(node.id, current.pos, current.size);
        }
        return null;
      });
    };
  ...
  // Dashed selection outline + 8 drag handles, rendered as a child of the
  // element itself so the handles line up with it regardless of nesting.
  const transformOverlay = isSelectedForTransform ? (
    <React.Fragment>
      <div
        style={{
          position: 'absolute',
      ...
      {(
        [
          { key: 'nw', style: { top: -12, left: -12, cursor: 'nwse-resize' } },
          { key: 'n', style: { top: -12, left: '50%', marginLeft: -12, cursor: 'ns-resize' } },
          { key: 'ne', style: { top: -12, right: -12, cursor: 'nesw-resize' } },
          { key: 'e', style: { top: '50%', right: -12, marginTop: -12, cursor: 'ew-resize' } },
          { key: 'se', style: { bottom: -12, right: -12, cursor: 'nwse-resize' } },
          { key: 's', style: { bottom: -12, left: '50%', marginLeft: -12, cursor: 'ns-resize' } },
          { key: 'sw', style: { bottom: -12, left: -12, cursor: 'nesw-resize' } },
          { key: 'w', style: { top: '50%', left: -12, marginTop: -12, cursor: 'ew-resize' } },
        ] as { key: GuiResizeHandle; style: React.CSSProperties }[]
      ).map((h) => (
        // Outer box is a 24x24 touch-friendly hit target; the visible dot
        // inside stays 8x8 so the on-screen look is unchanged.
        <div
          key={h.key}
          onPointerDown={(e) => startGuiDrag(e, h.key)}
          style={{
            position: 'absolute',
            width: 24,
            height: 24,
  ...
  const left = `calc(${pos.xScale * 100}% + ${pos.xOffset}px)`;
  const top = `calc(${pos.yScale * 100}% + ${pos.yOffset}px)`;
  const width = `calc(${sz.xScale * 100}% + ${sz.xOffset}px)`;
  const height = `calc(${sz.yScale * 100}% + ${sz.yOffset}px)`;
  const transform = (anchor[0] !== 0 || anchor[1] !== 0)
    ? `translate(${-anchor[0] * 100}%, ${-anchor[1] * 100}%)`
    : undefined;

  const bgTransparency = node.backgroundTransparency ?? 0;
  let bgColor = node.backgroundColor || '#ffffff';

  if (isInteractive && node.autoButtonColor !== false && (typeLower === 'textbutton' || typeLower === 'imagebutton')) {
    if (isPressed) {
      bgColor = mixHex(bgColor, '#000000', 0.25);
    } else if (isHovered) {
      bgColor = mixHex(bgColor, '#ffffff', 0.15);
    }
  }

  const bg = bgTransparency >= 1 ? 'transparent' : hexToRgba(bgColor, bgTransparency);
  const borderSize = node.borderSizePixel ?? 1;
  const border = borderSize > 0 ? `${borderSize}px solid ${node.borderColor || '#1b2a34'}` : 'none';
  const zIndex = node.zIndex ?? 1;
  const clips = !!node.clipsDescendants;
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    fireInstanceSignal(node.id, 'MouseButton1Click');
    fireInstanceSignal(node.id, 'Activated');
  };
  ...
  const handleMouseDown = (e: React.PointerEvent) => {
    ...
    setIsPressed(true);
    if (!isInteractive) return;
    fireInstanceSignal(node.id, 'MouseButton1Down', e.clientX, e.clientY);
  };
  ...
  const handleMouseUp = (e: React.PointerEvent) => {
    ...
    setIsPressed(false);
    if (!isInteractive) return;
    fireInstanceSignal(node.id, 'MouseButton1Up', e.clientX, e.clientY);
  };

  const handleMouseEnter = (e: React.MouseEvent) => {
    setIsHovered(true);
    if (!isInteractive) return;
    fireInstanceSignal(node.id, 'MouseEnter', e.clientX, e.clientY);
  };
  ...
  const handleMouseLeave = (e: React.MouseEvent) => {
    ...
    setIsPressed(false);
    if (!isInteractive) return;
    fireInstanceSignal(node.id, 'MouseLeave', e.clientX, e.clientY);
  };

  const renderChildren = () => (
    (node.children || [])
      .filter((c) => ['frame', 'textlabel', 'textbutton', 'imagelabel', 'imagebutton', 'textbox', 'scrollingframe'].includes((c.type || '').toLowerCase()))
      .map((child) => (
        <GuiElementNode
          key={child.id}
          node={child}
          ...
          onSelectGui={onSelectGui}
          onGuiTransformChange={onGuiTransformChange}
        />
      ))
  );
  ...
  const getTextStyle = (): React.CSSProperties => {
    ...
    return {
      ...
      height: '100%',
      display: 'flex',
      alignItems: node.textYAlignment === 'Top' ? 'flex-start' : node.textYAlignment === 'Bottom' ? 'flex-end' : 'center',
      justifyContent: node.textXAlignment === 'Left' ? 'flex-start' : node.textXAlignment === 'Right' ? 'flex-end' : 'center',
      textAlign: (node.textXAlignment || 'center').toLowerCase() as any,
      padding: '4px',
      userSelect: typeLower === 'textbox' ? 'text' : 'none',
      boxSizing: 'border-box',
    };
  };
  ...
  if (typeLower === 'imagelabel') {
    const imgOpacity = Math.max(0, Math.min(1, 1 - (node.imageTransparency ?? 0)));
    const objectFit = node.scaleType === 'Fit' ? 'contain' : node.scaleType === 'Crop' ? 'cover' : 'fill';
  ...
  if (typeLower === 'textbox') {
    return (
      <div style={baseStyle} onPointerDown={handleMouseDown}>
        <input
          type="text"
          value={node.text ?? ''}
          placeholder={node.placeholderText ?? 'Type here...'}
          ...
          style={{
            ...
            border: 'none',
            outline: 'none',
            cursor: editable ? 'move' : isInteractive ? 'text' : 'default',
            pointerEvents: editable ? 'none' : undefined,
          }}
          onFocus={() => {
            if (node.clearTextOnFocus && onUpdateText) {
              onUpdateText(node.id, '');
            }
          }}
          onChange={(e) => {
            if (onUpdateText) {
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              ...
              fireInstanceSignal(node.id, 'FocusLost', true);
            }
          }}
          onBlur={() => {
            fireInstanceSignal(node.id, 'FocusLost', false);
          }}
          onMouseEnter={handleMouseEnter}
          onMouseLeave={handleMouseLeave}
        />
  ...
  if (typeLower === 'scrollingframe') {
    const canvasH = node.canvasSize ? node.canvasSize[1] : 500;
    return (
      <div
        style={{
          ...baseStyle,
...
function ScreenGuiOverlay({
  ...
  activeTabId: string;
  gameplayMode: 'server' | 'client';
  isRunning: boolean;
  displayTree: TreeNodeData[];
  onUpdateText?: (nodeId: string, text: string) => void;
  activeTool?: 'select' | 'translate' | 'scale' | 'rotate' | 'guiTransform';
  selectedGuiId?: string | null;
  onSelectGui?: (nodeId: string) => void;
  onDeselectGui?: () => void;
  onGuiTransformChange?: (nodeId: string, guiPosition: GuiUDim2, guiSize: GuiUDim2) => void;
}) {
  const isEditMode = activeTabId === 'viewport';
  const isGameplayMode = activeTabId === 'gameplay';

  if (!isEditMode && !isGameplayMode) return null;
  ...
  // In Studio Edit mode, GUI is rendered as an edit preview, unless the
  // "Move/Resize GUI" tool is active, in which case it becomes editable.
  const isInteractive = isGameplayMode && gameplayMode === 'client';
  const editable = isEditMode && activeTool === 'guiTransform';

  // Collect ScreenGui nodes
  let screenGuis: TreeNodeData[] = [];
  const starterGui = displayTree.find((n) => n.id === 'startergui' || n.name.toLowerCase() === 'startergui');
  if (starterGui && starterGui.children) {
    screenGuis.push(...starterGui.children.filter((c) => (c.type || '').toLowerCase() === 'screengui' && c.enabled !== false));
  }

  // Also collect any ScreenGuis under Players (e.g. PlayerGui)
  const players = displayTree.find((n) => n.id === 'players' || n.name.toLowerCase() === 'players');
  if (players && players.children) {
    const traversePlayers = (nodes: TreeNodeData[]) => {
      for (const p of nodes) {
        if (p.name.toLowerCase() === 'playergui') {
          screenGuis.push(...p.children.filter((c) => (c.type || '').toLowerCase() === 'screengui' && c.enabled !== false));
        }
        if (p.children && p.children.length > 0) traversePlayers(p.children);
      }
    };
    traversePlayers(players.children);
  }

  if (screenGuis.length === 0) return null;

  screenGuis.sort((a, b) => (a.displayOrder ?? 0) - (b.displayOrder ?? 0));

  return (
    <div
      className={`absolute inset-0 overflow-hidden select-none z-20 ${editable ? '' : 'pointer-events-none'}`}
      onPointerDown={() => {
        // Clicking/tapping empty space (not any GUI element) deselects,
        // since elements themselves stop propagation before this can fire.
        if (editable) onDeselectGui?.();
      }}
    >
      {screenGuis.map((gui) => (
        <div
          ...
          style={{
            zIndex: gui.displayOrder ?? 1,
            pointerEvents: editable ? 'auto' : 'none',
          }}
        >
          {(gui.children || []).map((child) => (
            <GuiElementNode
              key={child.id}
              node={child}
              isInteractive={isInteractive}
...
export default function App() {
  ...
  const [activeTool, setActiveTool] = useState<'select' | 'translate' | 'scale' | 'rotate' | 'guiTransform'>('select');
  ...
  const insertObject = (
    ...
    if (objectType === 'model') {
      ...
      name = 'Frame';
    } else if (objectType === 'textlabel') {
      name = 'TextLabel';
    } else if (objectType === 'textbutton') {
      name = 'TextButton';
    } else if (objectType === 'imagelabel') {
      name = 'ImageLabel';
    } else if (objectType === 'imagebutton') {
      name = 'ImageButton';
    } else if (objectType === 'textbox') {
      name = 'TextBox';
    } else if (objectType === 'scrollingframe') {
      name = 'ScrollingFrame';
    } else if (objectType === 'surfacegui') {
      name = 'SurfaceGui';
    ...
    const newNode: TreeNodeData = {
      ...
      color: isDecalOrTexture ? '#ffffff' : (isNon3d ? undefined : '#a3a2a5'),
      anchored: isNon3d ? undefined : false,
      canCollide: isNon3d ? undefined : true,
      canTouch: isNon3d ? undefined : true,
      // GUI Base Properties
      guiPosition: isGui && !['screengui', 'surfacegui', 'billboardgui'].includes(objectType)
        ? { xScale: 0, xOffset: 0, yScale: 0, yOffset: 0 }
        : undefined,
      guiSize: ['frame', 'imagelabel', 'imagebutton'].includes(objectType)
        ...
        : undefined,
      anchorPoint: isGui && !['screengui', 'surfacegui', 'billboardgui'].includes(objectType)
        ? [0, 0]
        : undefined,
      backgroundColor: objectType === 'textbutton' || objectType === 'imagebutton'
        ? '#0078d7'
        : ['frame', 'screengui'].includes(objectType)
        ? '#ffffff'
        : ['textlabel', 'imagelabel', 'scrollingframe'].includes(objectType)
        ? '#2b2b2b'
        : objectType === 'textbox'
        ? '#1e1e1e'
        : undefined,
      ...
      active: ['textbutton', 'imagebutton', 'textbox'].includes(objectType) ? true : undefined,
      zIndex: isGui ? 1 : undefined,
      clipsDescendants: objectType === 'scrollingframe' ? true : false,
      // Text
      text: objectType === 'textlabel' ? 'Label' : objectType === 'textbutton' ? 'Button' : objectType === 'textbox' ? '' : undefined,
      textColor: ['textlabel', 'textbutton', 'textbox'].includes(objectType) ? '#ffffff' : undefined,
      textTransparency: ['textlabel', 'textbutton', 'textbox'].includes(objectType) ? 0 : undefined,
      textSize: ['textlabel', 'textbutton', 'textbox'].includes(objectType) ? 14 : undefined,
      textScaled: false,
      textWrapped: true,
      font: ['textlabel', 'textbutton', 'textbox'].includes(objectType) ? 'SourceSans' : undefined,
      textXAlignment: 'Center',
      textYAlignment: 'Center',
      // Image
      image: ['imagelabel', 'imagebutton'].includes(objectType) ? '' : undefined,
      imageColor: ['imagelabel', 'imagebutton'].includes(objectType) ? '#ffffff' : undefined,
      imageTransparency: ['imagelabel', 'imagebutton'].includes(objectType) ? 0 : undefined,
      scaleType: ['imagelabel', 'imagebutton'].includes(objectType) ? 'Stretch' : undefined,
      // Button
      autoButtonColor: ['textbutton', 'imagebutton'].includes(objectType) ? true : undefined,
      // TextBox
      placeholderText: objectType === 'textbox' ? 'Placeholder Text' : undefined,
      clearTextOnFocus: objectType === 'textbox' ? true : undefined,
      // ScreenGui
      ignoreGuiInset: objectType === 'screengui' ? false : undefined,
      resetOnSpawn: objectType === 'screengui' ? true : undefined,
      displayOrder: objectType === 'screengui' ? 0 : undefined,
      // 3D GUIs
      face: objectType === 'surfacegui' ? 'Front' : (isDecalOrTexture ? 'Front' : ((objectType === 'spotlight' || objectType === 'surfacelight') ? 'Front' : undefined)),
      canvasSize: objectType === 'surfacegui' ? [800, 600] : objectType === 'billboardgui' ? [200, 50] : undefined,
      studsOffset: objectType === 'billboardgui' ? [0, 2, 0] : undefined,
      alwaysOnTop: objectType === 'billboardgui' ? true : false,
      maxDistance: objectType === 'billboardgui' ? 100 : undefined,
  ...
  const getNodeIcon = (name: string, type: string) => {
    ...
    if (nodeType === 'textbutton') return <MousePointer {...getProps('text-[#fbbf24]')} />;
    if (nodeType === 'imagelabel') return <ImageIcon {...getProps('text-[#f472b6]')} />;
    if (nodeType === 'imagebutton') return <MousePointerClick {...getProps('text-[#f43f5e]')} />;
    if (nodeType === 'textbox') return <TextCursor {...getProps('text-[#c084fc]')} />;
  ...
  const handleGuiTextUpdate = (nodeId: string, text: string) => {
    const updateNode = (nodes: TreeNodeData[]): TreeNodeData[] => {
      return nodes.map((n) => {
        if (n.id === nodeId) return { ...n, text };
        if (n.children.length > 0) return { ...n, children: updateNode(n.children) };
        return n;
      });
    };
    if (isRunning && activeTabId === 'gameplay') {
      setClientRuntimeTree((prev) => updateNode(prev));
    } else {
        ...
        <div className="h-[40px] flex-shrink-0 flex items-center justify-between px-3 bg-[#2c2c2c] border-b border-[#050505]">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-[#1e1e1e] p-0.5 rounded-[3px] border border-[#3e3e42]">
              ...
              <div className="w-[1px] h-[18px] bg-[#3e3e42] mx-0.5"></div>
              <button 
                onClick={() => setActiveTool('guiTransform')}
                className={`p-1.5 rounded-sm transition-colors ${activeTool === 'guiTransform' ? 'bg-[#0078d7] text-white' : 'text-[#cccccc] hover:bg-[#333333]'}`}
                title="Move/Resize GUI (2D)"
              >
                <Frame size={14} />
              </button>
            </div>
          ...
          {activeTabId === 'viewport' || activeTabId === 'gameplay' ? (
            ...
            <div className="flex-1 relative overflow-hidden bg-[#0b1220]">
              ...
              <ScreenGuiOverlay
                ...
                onSelectGui={(nodeId) => setSelectedId(nodeId)}
                onDeselectGui={() => setSelectedId(null)}
                onGuiTransformChange={(nodeId, guiPosition, guiSize) => {
                  const updateNode = (nodes: TreeNodeData[]): TreeNodeData[] =>
                    nodes.map((n) => {
                      if (n.id === nodeId) return { ...n, guiPosition, guiSize };
                      return { ...n, children: updateNode(n.children) };
                    });
                  updateActiveTree((prev) => updateNode(prev));
                }}
