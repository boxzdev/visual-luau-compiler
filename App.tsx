...
function GuiElementNode({
  ...
  const startGuiDrag = (e: React.MouseEvent, handle: GuiResizeHandle) => {
    e.stopPropagation();
    e.preventDefault();
    onSelectGui?.(node.id);
  ...
  const transformOverlay = isSelectedForTransform ? (
    <React.Fragment>
      ...
      {(
        ...
        <div
          key={h.key}
          onMouseDown={(e) => startGuiDrag(e, h.key)}
          style={{
            position: 'absolute',
            width: 8,
            height: 8,
...
function ScreenGuiOverlay({
  ...
  return (
    <div
      ...
      onMouseDown={() => {
        // Clicking empty space (not any GUI element) deselects, since
        // elements themselves stop propagation before this can fire.
